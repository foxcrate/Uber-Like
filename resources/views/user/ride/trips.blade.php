                ...
                <table class="table table-condensed" style="border-collapse:collapse">
                    ...
                    @foreach($trips as $trip)
                        ...
                        <tr data-toggle="collapse" data-target="#trip_{{$trip->id}}" class="accordion-toggle collapsed">
                            ...
                            {{-- @if($trip->provider)
                                <td>{{base64_decode($trip->provider->first_name)}} {{base64_decode($trip->provider->last_name)}}</td>
                            @else
                                <td>-</td>
                            @endif --}}
                            @if($trip->payment)
                                ...
                                <div class="accordian-body collapse row" id="trip_{{$trip->id}}">
                                    ...
                                    $static_map = "https://maps.googleapis.com/maps/api/staticmap?autoscale=1&size=600x450&maptype=terrain&format=png&visual_refresh=true&markers=icon:".$map_icon."%7C".$trip->s_latitude.",".$trip->s_longitude."&markers=icon:".$map_icon."%7C".$trip->d_latitude.",".$trip->d_longitude."&path=color:0x191919|weight:8|enc:".$trip->route_key."&key=".env('GOOGLE_MAP_KEY'); ?>
                                            ...
                                            <div class="from-to row no-margin">
                                                <div class="from">
                                                    ...
                                                    <h6>{{date('H:i A - d-m-y', strtotime($trip->created_at))}}</h6>
                                                    <p>{{$trip->s_address}}</p>
                                                </div>
                                                <div class="to">
                                                    <h5>@lang('user.to')</h5>
                                                    <h6>{{date('H:i A - d-m-y', strtotime($trip->arrived_at))}}</h6>
                                                    <p>{{$trip->d_address}}</p>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <div class="mytrip-right">
                                            ...
                                            <div class="trip-user">
                                                <div class="pro-img">
                                                    <img style="width: 100%;height: 100%;border-radius: 50%;" src="{{asset($trip->provider['avatar'])}}">
                                                </div>


                                                <div class="user-right">
                                                    @if($trip->provider)
                                                        <h5>{{base64_decode($trip->provider->first_name)}} {{base64_decode($trip->provider->last_name)}}</h5>
                                                    @else
                                                    <h5>- </h5>
                                                    @endif
                                                    @if($trip->rating)
