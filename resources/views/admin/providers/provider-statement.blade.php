									...
									<div class="box bg-white">
										...
										@if(count($Providers) != 0)
								            <table class="table table-striped table-bordered dataTable" id="table-2">
								                <thead>
								                   <tr>
														<td>@lang('admin.Provider_Name')</td>
														<td>@lang('admin.Mobile')</td>
														<td>@lang('admin.Status')</td>
														<td>@lang('admin.Total_Rides')</td>
														<td>@lang('admin.Total_Earning')</td>
														<td>@lang('admin.Commission')</td>
														<td>@lang('admin.Joined_at')</td>
														<td>@lang('admin.Details')</td>
													</tr>
								                </thead>
								                <tbody>
								                <?php $diff = ['-success','-info','-warning','-danger']; ?>
														@foreach($Providers as $index => $provider)
															<tr>
																...
																<td>
																	<a href="{{route('admin.provider.statement', $provider->id)}}">@lang('admin.View_by_Ride')</a>
																</td>
															</tr>
														@endforeach
