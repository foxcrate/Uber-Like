    ...
    public function accepted(Request $request)
    {
        ...
        if ($trip) {
            if ($trip->status == 'SEARCHING') {
                $trips = $trip->update([
                    'status' => 'ACCEPTED',// وافق
                    'provider_id' => $request->user()->id
                ]);
                return responseJson(1, 'تم الموافقه على الطلب من السائق ' . $request->user()->first_name .
                    ' ' . $request->user()->first_name, ['trip' => $trip]);
            } elseif ($request->status == 1 && $trip->status == 'ACCEPTED') { //1 -> 'ACCEPTED' => 'PICKEDUP'
                $trips = $trip->update([
                    'status' => 'PICKEDUP',
                ]);
                return responseJson(1, 'المسافر تم التقاطه', ['trip' => $trip]);

            } elseif ($request->status == 2 && $trip->status == 'ACCEPTED') { //2 -> 'ACCEPTED' -> 'SEARCHING'
                $trips = $trip->update([
                    'status' => 'SEARCHING',
                ]);
                return responseJson(1, 'تم الطلب تم الغاء الموافقه منه', ['trip' => $trip]);

            } elseif ($request->status == 3 && $trip->status == 'PICKEDUP') { //3 -> 'PICKEDUP' => 'COMPLETED'

                $trips = $trip->update([
                    'status' => 'COMPLETED',
                ]);
                return responseJson(1, 'هذا الطلب اكتمل', ['trip' => $trip]);

            } elseif ($request->status == 4 && $trip->status == 'COMPLETED' && $trip->payment_mode == 'CASH') { //4 -> 'COMPLETED' => paid = 1

                $trips = $trip->update([
                    'paid' => 1,
                ]);
                return responseJson(1, 'تمت عمليه الدفع', ['trip' => $trip]);

            } else {
                if ($trip->status != 'PICKEDUP') {
                    return responseJson(0, 'هذا الطلب لا يمكن ان يكتمل');
                } elseif ($request->status == 1 && $trip->status != 'ACCEPTED') {
                    return responseJson(0, 'هذا الطلب لا يمكن الموافقه عليه');
                } elseif ($request->status == 2 && $trip->status != 'ACCEPTED') {
                    return responseJson(0, 'هذا الطلب لا يمكن الغائه');
                }
...
//                    'provider_id' => $request->user()->id
//                ]);
//                return responseJson(1, 'تم الارسال بنجاح', ['trip' => $trip]);
//            }
...
//
    ...
    public function index(Request $request)
    {
        try {
            ...
            return response()->json(['error' => trans('admin.Something Went Wrong')]);
        }
    }
    ...
    {
        $UserRequest = UserRequests::findOrFail($request->request_id)->update(['distance' => $request->distance]);
        return response()->json(['message' => $request->request_id]);
    }
    ...
    public function cancel(Request $request)
    {
        ...
        try {
            ...
            if ($UserRequest->status == "CANCELLED") {
                return responseJson(0, 'لا يمكن الغاء هذا الطلب');
            }
    ...
    public function rate(Request $request, $id)
    {
        ...
        try {
            ...
            return response()->json(['message' => trans('admin.createMessageSuccess')]);

        } catch (ModelNotFoundException $e) {
            return response()->json(['error' => trans('admin.Something Went Wrong')], 500);
        }
    }
    ...
    public function history(Request $request)
    {
        ...
        //if ($request->ajax()) {
            ...
            //$tripsx = UserRequests::where('provider_id', auth()->user()->id)->get();
            $trips = UserRequests::ProviderTrips(auth()->user()->id)->get();
            ...
            //return $trips[0];
        //}
        //$trips = UserRequests::where('provider_id', Auth::guard('provider')->user()->id)->with('user', 'service_type', 'payment', 'rating')->get();
        //return compact('trips');
        //return auth()->user()->first_name;
        return view('provider.trip.index', compact('trips'));
    }
    ...
    public function accept(Request $request, $id)
    {
        try {
            ...
            if ($UserRequest->status != "SEARCHING") {
                return response()->json(['error' => trans('admin.Request already under progress')]);
            }
            ...
            if ($UserRequest->schedule_at != "") {
                ...
                if ($CheckScheduling > 0) {
                    if ($request->ajax()) {
                        ...
                        return redirect('dashboard')->with('flash_error', trans('admin.If the ride is already scheduled then we cannot schedule/request another ride for the after 1 hour or before 1 hour'));
                    }
                }
            ...
            return response()->json(['error' => trans('admin.Unable to accept, Please try again later')]);
        } catch (Exception $e) {
            return response()->json(['error' => trans('admin.Something Went Wrong')]);
        }
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            return response()->json(['error' => trans('admin.Unable to update, Please try again later')]);
        } catch (Exception $e) {
            return response()->json(['error' => trans('admin.Something Went Wrong')]);
        }
    }
    ...
    public function destroy($id)
    {
        ...
        try {
            ...
            return response()->json(['error' => trans('admin.Unable to reject, Please try again later')]);
        } catch (Exception $e) {
            return response()->json(['error' => trans('admin.Something Went Wrong')]);
        }
    }
