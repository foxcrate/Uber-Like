    ...
    public function history(Request $request)
    {
        ...
        if (auth()->user()->device_mac == $request->device_mac) {
            ...
            if ($request->ajax()) {
                ...
                $Jobs = UserRequests::where('provider_id', Auth::user()->id)
                    ->whereIn('status' , ['CANCELLED', 'COMPLETED', 'SCHEDULED'])
                    //->whereIn('cancelled_by' , ['NONE', 'PROVIDER'])
    ...
    public function summary(Request $request)
    {
        ...
        if (auth()->user()->device_mac == $request->device_mac) {
            ...
            try {
                ...
                if ($request->lang == "ar") {
                    return response()->json(['message' => "Something Went Wrong Ar"], 400);
                } elseif ($request->lang == "en") {
                    return response()->json(['message' => "Something Went Wrong En"], 400);
                } else {
                    return response()->json(['message' => "Something Went Wrong Ar"], 400);
                }
            }
